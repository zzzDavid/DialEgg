module {
  ...
  func.func @main() -> i32 {
    ...
    %0 = tensor.empty(%c100, %c10) : tensor<?x?xi64>
    %1 = tensor.empty(%c10, %c150) : tensor<?x?xi64>
    %2 = tensor.empty(%c150, %c8) : tensor<?x?xi64>
    %3 = call @fillRandomI64Tensor2D(%0) : (tensor<?x?xi64>) -> tensor<?x?xi64>
    %4 = call @fillRandomI64Tensor2D(%1) : (tensor<?x?xi64>) -> tensor<?x?xi64>
    %5 = call @fillRandomI64Tensor2D(%2) : (tensor<?x?xi64>) -> tensor<?x?xi64>
    %cast = tensor.cast %3 : tensor<?x?xi64> to tensor<100x10xi64>
    %cast_0 = tensor.cast %4 : tensor<?x?xi64> to tensor<10x150xi64>
    %cast_1 = tensor.cast %5 : tensor<?x?xi64> to tensor<150x8xi64>
    %6 = call @clock() : () -> i64
    %7 = call @_2mm(%cast, %cast_0, %cast_1) : (tensor<100x10xi64>, tensor<10x150xi64>, tensor<150x8xi64>) -> tensor<100x8xi64>
    %8 = call @clock() : () -> i64
    %cast_2 = tensor.cast %7 : tensor<100x8xi64> to tensor<?x?xi64>
    call @printI64Tensor2D(%cast_2) : (tensor<?x?xi64>) -> ()
    call @printNewline() : () -> ()
