...
# clear time.txt
open("res/time.txt", "w").close()

def eq_sat_time(file_path, interest_function, data):
        ...
        for i, line in enumerate(lines):
            if line == f"Done running on function: {interest_function}":
                for j, line in enumerate(lines[i:i+7]):
                    for keyword in ['mlirToEgglogWallTime', 'egglogWallTime', 'egglogToMlirWallTime']:
                        if keyword in line:
...
def main():
    ...
    # eq_sat_time("bench/arith_rgb_to_gray.mlir", "rgb_to_grayscale", data)
    # eq_sat_time("bench/linalg_assoc.mlir", "main", data)
    # eq_sat_time("bench/linalg_3mm.mlir", "main", data)
    # eq_sat_time("bench/math_inv_sqrt.mlir", "normalize_vector", data)
    # eq_sat_time("bench/math_horners_method.mlir", "poly_eval_3", data)

    # cpp_time("bench/linalg_assoc.mlir", data)
    # cpp_time("bench/linalg_3mm.mlir", data)

    # canon_time("bench/arith_rgb_to_gray.mlir", data)
    # canon_time("bench/linalg_assoc.mlir", data)
    # canon_time("bench/linalg_3mm.mlir", data)
    # canon_time("bench/math_inv_sqrt.mlir", data)
    # canon_time("bench/math_horners_method.mlir", data)
